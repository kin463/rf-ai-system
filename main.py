...
def get_relevant_sections(user_query: str, all_text: str) -> str:
    ...
    for section in sections:
        ...
        # 1. 人名検索（苗字のみでもヒットするように）
        for name in names:
            if name in user_query: score += 100
            elif len(name) > 2 and name[:2] in user_query: score += 60
        
        # 2. キーワード検索
        keywords = ["帰社日", "勉強会", "有給", "休暇", "連絡", "提出", "期限", "手当", "所属", "賞与", "寝坊", "遅刻"]
        if any(kw in user_query for kw in keywords): score += 70
        
        # 3. 組織キーワード
        if "RandF" in user_query and "RandF" in section: score += 80
            
        if score > 0:
            scored_sections.append((score, section))
            
    scored_sections.sort(key=lambda x: x[0], reverse=True)
    selected = "\n\n".join([s[1] for s in scored_sections[:10]])
    return selected if selected else all_text[:5000]
...
async def chat(request: ChatRequest):
    ...
    【回答ガイドライン】
    1. ユーザーが用語を間違えている場合（例：「就労手当」→「資格手当」等）、資料から正しい項目を推測して回答してください。
    2. 資料に情報がない場合は憶測せず「記載がありません」と報告してください。
    3. 同じ説明を繰り返さないでください。
    4. 帰社日や連絡ルールについては、必ず【社内資料】の最新のセクションを参照してください。
